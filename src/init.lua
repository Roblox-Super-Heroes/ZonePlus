...
function Zone:getCheckerPart()
	-- Wait if another process is already reparenting the checker part
	if self._checkerPartLock then
		repeat
			task.wait()
		until not self._checkerPartLock
	end

	local checkerPart = self.checkerPart
	if not checkerPart then
		checkerPart = Instance.new("Part")
		...
		self.janitor:add(checkerPart, "Destroy")
	end

	-- Lock the reparenting section
	self._checkerPartLock = true
	...
	if checkerPart.Parent ~= checkerParent then
		-- Try setting the parent in a protected call
		local success, err = pcall(function()
			checkerPart.Parent = checkerParent
		end)
		-- If it fails, log the error, wait briefly, then try again
		if not success then
			warn("Failed to set Parent of CheckerPart: " .. err)
			task.wait(0.1)
			pcall(function()
				checkerPart.Parent = checkerParent
			end)
		end
		-- Double-check if it is now correctly parented
		if checkerPart.Parent ~= checkerParent then
			warn("CheckerPart's parent is still not set correctly after retry.")
		end
	end

	self._checkerPartLock = false
...
function Zone:_getAll(trackerName)
	...
	local zonesAndOccupants =
		ZoneController._getZonesAndItems(trackerName, { self = true }, self.volume, false, self._currentEnterDetection)
